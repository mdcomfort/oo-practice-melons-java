	...
	public static void getSellabilityReport(Melon[] melons) {
		for (int i = 0; i < melons.length; i++) {
			if (melons[i].isSellable()) {
				System.out.println("Harvested by " + melons[i].harvester + " from FIELD " + melons[i].field + " (CAN BE SOLD)");
			}
			else {
				System.out.println("Harvested by " + melons[i].harvester + " from FIELD " + melons[i].field + " (NOT SELLABLE)");
			}
		}
	}
}
